...
# ----------------------------------------------------------------------------------------------------------------------------
    ...
    for i in range(0, len(Time)):
        ...
        for j in range(0, len(S0_d)):
            ...
            price_euro[i, j], Delta_euro[i, j], gamma_euro[i, j], theta_euro[i, j], __ = Tree.call_CallOption_tree()

            Tree = BinomialTree(S0_d[j], K1, T1, K2, T2, sigma, r, delta, n, option='amer')
            price_amer[i, j], Delta_amer[i, j], gamma_amer[i, j], theta_amer[i, j], __ = Tree.call_CallOption_tree()
...
#     # Greeks Psi for Derivative Y in the European case
    ...
    for i in range(0, len(Time)):
        ...
        for j in range(0, len(S0_p)):
            ...
            print('Completed calculation of Psi with T_1 = T_2 = {:.2f}, S0 = {:.2f}.'.format(Time[i], S0_p[j]))
